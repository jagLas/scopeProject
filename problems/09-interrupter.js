...
const interrupter = interruptingWord =>{
  return string => {
    let words = string.split(' ') //splits words into array

    for (let i = 0; i < words.length - 1; i++){ //iterrates through each word except the last
      let word = words[i];
      let newWord = word + ' ' + interruptingWord;
